...
# Fix Windows console Unicode encoding (only when stdout has a buffer, not in Streamlit)
...
# Define agents (cached so they're only initialized once per session)
@st.cache_resource
def get_agents():
    ...
    return reporter_agent, travel_agent, web_research_agent

reporter_agent, travel_agent, web_research_agent = get_agents()
...
def format_markdown_images(markdown_text):
    ...
    Also sanitizes non-encodable Unicode characters for Windows compatibility.
    """
    if not isinstance(markdown_text, str):
        markdown_text = str(markdown_text)

    # Encode to cp1252 and back to drop characters Windows can't handle in logs
    # (Streamlit itself handles full Unicode fine — this only affects the console)
        ...
        if current_location and destination and dates:
            try:
                ...
                for i, (key, (title, func)) in enumerate(sections.items()):
                    if i > 0:
                        time.sleep(10)  # avoid Groq rate limits between agent calls
                    with st.container():
